...
const Dashboard: React.FC<DashboardProps> = ({ data }) => {
  ...
  const totalCapital = totalMonthlyIncome + totalSavings - totalDebt;
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      {/* Hero: Total Capital (Inspired by N26 Balance Card) */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Sources Segmented Bar (Inspired by "Sources" component) */}
          ...
          <div>
            ...
            <div className="space-y-4">
              {[
                ...
                { label: 'Investments', amount: 0, color: 'bg-[#8B5CF6]' }
              ].map((item, i) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Expenses Analytics (Inspired by Donut Charts) */}
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm relative">
          ...
          <div className="h-[320px] relative">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <p className="text-3xl font-black text-slate-900 tracking-tighter">${totalOutgoings.toLocaleString()}</p>
            ...
            {categorySpending.slice(0, 4).map((cat, i) => (
              <div key={i} className="flex items-center space-x-3 p-3 rounded-2xl bg-slate-50/50">
                ...
                <div className="min-w-0">
                  ...
                  <p className="font-bold text-slate-900 truncate">${cat.value}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Recent Transactions (Inspired by Transaction List) */}
            ...
            {data.outgoings.slice(-6).reverse().map((item) => (
              <div key={item.id} className="flex items-center justify-between group cursor-pointer">
                ...
                <div className="text-right">
                  <p className="font-black text-rose-600 text-sm">-${item.amount.toLocaleString()}</p>
                </div>
              </div>
            ))}
          </div>
